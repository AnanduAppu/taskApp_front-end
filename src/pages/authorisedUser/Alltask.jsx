...
function Alltask() {
  const { AuthtaskData, SetauthTaskData } = useContext(UserContex);
  ...
  const currentTasks = AuthtaskData.slice(startIndex, endIndex);
  ...
  const openModalview = (id) => {
    const taskView = AuthtaskData.find((ele) => ele._id == id);
  ...
  const submitDelete = async (e) => {
    ...
    try {
      const responseDele = await axios.delete(
        "http://localhost:3020/user/deletetask",
        {
          data: { taskid: taskid },
        }
      );
      if (responseDele.data.success) {
        SetauthTaskData((prevAuthtaskData) =>
          prevAuthtaskData.filter((task) => task._id !== taskid)
        );
        toast.success("deleted successful");
        setTaskId("");
      }
    } catch (error) {
      toast.error("an error occcured deleting");
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const responseEdit = await axios.put(
        "http://localhost:3020/user/edittask",
        { dateEdit, headingEdit, descriptionEdit, priorityEdit, taskid }
      );

      if (responseEdit.data.success) {
        // Update the taskData state with the edited task
        SetauthTaskData((prevAuthtaskData) =>
          prevAuthtaskData.map((task) =>
            task._id === taskid
              ? {
                  ...task,
                  date: dateEdit,
      ...
      toast.error("an error occcured in editing");
    }
  };
      ...
      <main className="ml-60 pt-16 max-h-screen overflow-auto">
              ...
              <div className="w-full">
                <div className="border-b border-gray-200 shadow">
                  ...
                  2.828L11.828 15H9v-2.828l8.586-8.586z"
                              ...
                              <td className="px-6 py-4">
                                <button
                                  onClick={() => openModelDelete(ele._id)}
                                >
                                  <svg
                                    xmlns="http://www.w3.org/2000/svg"
        ...
        {isModalEdit && (
          <div
            id="medium-modal"
            tabIndex="-1"
            className="fixed top-0 left-0 right-0 z-50 w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full ms-96 mt-10"
          >
        ...
        {deleteModel && (
          <div
            id="popup-modal"
            tabIndex="-1"
            className="overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full mx-96 mt-10"
          >
